# Callsign — Agent Operating Instructions

## 1. Project summary

Callsign is a peer-to-peer consultation primitive for coding agents: it
lets your Claude Code agent ask a specific person's agent a question and
get back only a pre-approved, explicitly human-marked-shareable slice of
their context — never a raw memory dump, never an LLM-inferred
disclosure. Current stage: pre-implementation. `PRD.md` exists; no code
...
- Revoking a standing grant takes effect immediately: grant validity MUST
  ...
  intake.
- Callsign must never import, submodule, or otherwise depend on the
  vaultd repo — they are separate projects by design. Identity/signing is
  Callsign's own standalone Rust module, modeled on vaultd's
  architectural pattern but independent code and repo.
...
- **Local (each person's machine):** Callsign MCP server/client,
  identity/signing (own standalone Rust module, pattern modeled on
  vaultd but independent code — never a dependency on the vaultd repo
  itself), capsule store (markdown + SQLite/FTS5), deterministic scope
  resolver, terminal approval UI, local audit log. All capsule content
  and private keys live here and never leave.
...
- Identity/signing: own standalone Rust module within the Callsign repo
...
- **Rust (identity module):** follow vaultd's proven daemon/binary split
  ...
  code owned entirely by the Callsign repo — do not import, submodule,
  or otherwise depend on the vaultd repo itself.
...
- No VerifiedAgent — a separate, related-but-distinct product idea, not
  a Callsign feature at any stage.
