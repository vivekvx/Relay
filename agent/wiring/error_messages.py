...
_FIXES = {
    "invalid_signature": (
        "Your local key doesn't match what's registered. Run `callsign mynumber` to check, "
        "then `callsign setup --force` to fix."
    ),
    "unknown_recipient": "That handle isn't registered on this registry yet. Ask them to run `callsign setup`.",
    "unknown_sender": "Your handle isn't registered. Run `callsign setup` first.",
    ...
    "registry_unreachable": "Can't reach the registry. Check your network, or run `callsign signal` for a full diagnosis.",
