...
def register_identity(
    ...
    try:
        relay_number = identity_service.register_identity(
    ...
    session.commit()
    return {"handle": body.handle, "relay_number": relay_number}


# Contacts (agent/wiring/contacts.py) resolve a saved local name to a
# relay_number, then need this endpoint to resolve THAT to the actual
# handle — a relay_number is routing metadata, same category as handle
# (registry/ARCHITECTURE.md's hard boundary; see schema.sql's comment
# on why it lives in this same table). Mirrors GET /{handle} below
# exactly, just keyed the other way.
#
# MUST be declared before GET /{handle} — FastAPI/Starlette matches
# routes in registration order, and /{handle} is a single-segment
